...
const ExpenseCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addToast(`Expense for "${formData.Merchant}" created!`, 'success');
      navigate('/');
    } catch (err) {
      console.error('Create expense error:', err);
      const msg = err.response?.data?.error || err.response?.data?.message || 'Failed to create expense.';
      setError(msg);
      addToast(msg, 'error');
    } finally {
      setLoading(false);
    ...
    <div style={{ maxWidth: '640px', margin: '0 auto' }}>
      <div className="page-header" style={{ marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h1 style={{ fontSize: '1.5rem', margin: 0 }}>Add New Expense</h1>
            <p className="subtitle">Record your recent transaction</p>
          </div>
        </div>
      </div>
